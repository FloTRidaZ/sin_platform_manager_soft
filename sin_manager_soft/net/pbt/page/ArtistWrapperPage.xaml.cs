...
{
    public sealed partial class ArtistWrapperPage
    {
        private readonly ObservableCollection<Artist> _artists;
        private readonly ResourceLoader _resourceLoader;

        public ArtistWrapperPage()
        {
            this.InitializeComponent();
            _resourceLoader = ResourceLoader.GetForCurrentView();
            _artists = SinCollection.GetLocalCollection().ArtistList;
        }

        private void ArtistWrapperContainerContentChanging(ListViewBase sender, ContainerContentChangingEventArgs args)
        {
            if (args.Phase != 0)
            {
                return;
        ...
        {
            Connector.GetInstance().SendArtistList();
        }
    }
}